  ...
  return (
    <div className="min-h-screen bg-brand-surface">
      ...
      <main id="main-content" className="mx-auto max-w-5xl px-4 py-10 md:py-14">
        <header>
          <h1 className="text-3xl font-bold text-brand-navy">{pillar.label}</h1>
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-3">
          <Link
            href="/events"
            className="rounded-2xl border bg-white p-5 text-center font-bold text-brand-navy hover:border-brand-saffron/40"
          >
            Events
          </Link>
          <Link
        ...
        <section className="mt-10" aria-labelledby="by-year">
          ...
          {CONFERENCE_YEAR_ARCHIVE.map((edition) => (
            <div key={edition.year} className="mt-6 rounded-2xl border bg-white p-5">
              ...
              <ul className="mt-3 space-y-1">
                {edition.routes.map((r) => (
                  <li key={r.path}>
                    <Link href={r.path} className="text-sm text-brand-navy hover:underline">
                      {r.label}
                    </Link>
                  </li>
                ))}
        ...
        <section className="mt-10" aria-labelledby="summits-list">
          ...
          <ul className="mt-3 space-y-2">
            {SUMMIT_ROUTES.map((r) => (
              <li key={r.path}>
                <Link href={r.path} className="font-medium hover:underline">
                  {r.label}
                </Link>
              </li>
            ))}
      ...
      <RelatedContentSection path="/conferences" />
      <Footer />
    </div>
  );
}
