...
/** Zero-JS en/hi switcher — plain links, no next-intl client bundle. */
export default async function NavLanguageLinks({ className = "" }: Props) {
  ...
  return (
    <nav aria-label="Language" className={className}>
      <div className="flex min-h-11 min-w-[7.5rem] items-center gap-0.5 rounded-lg border border-slate-200 bg-white p-1">
        <Link href={enHref} className={pillClass(current === "en")} aria-current={current === "en" ? "page" : undefined}>
          English
        </Link>
        <Link href={hiHref} className={pillClass(current === "hi")} aria-current={current === "hi" ? "page" : undefined}>
          हिन्दी
        </Link>
      </div>
    </nav>
