...
/** Oxford, Boston University, and similar overseas institutions — excludes SAU Delhi */
export function isInternationalDesignation(designation: string): boolean {
  const d = designation.toLowerCase();
  return /university of oxford|boston university/i.test(d);
}

export function isInternationalWish(wish: BestWishEntry): boolean {
  return isInternationalDesignation(wish.designation);
}

export const BEST_WISHES_STATS = {
  total: BEST_WISHES_ENTRIES.length,
  featured: BEST_WISHES_ENTRIES.filter((w) => w.featured).length,
  international: BEST_WISHES_ENTRIES.filter((w) => isInternationalWish(w)).length,
} as const;
...
export const WISH_EDITION_FILTERS = buildWishEditionFilters();
export type WishEditionFilter = (typeof WISH_EDITION_FILTERS)[number];

/** Edition filter chips derived from archived message tags */
export function buildWishEditionFilters(): readonly string[] {
  const editions = Array.from(new Set(BEST_WISHES_ENTRIES.map((w) => w.edition))).sort(
    (a, b) => {
      if (a === "Abhiyan") return -1;
      if (b === "Abhiyan") return 1;
