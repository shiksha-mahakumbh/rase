...
const Info = () => {
  const text = `
  <h2 class="text-3xl font-bold text-[#502a2a] mb-4 text-center">
  ...
  const textClassHeading = isMobile ? "text-xl" : "text-3xl";

  return (
    <div className="bg-gradient-to-br from-[#fff7f7] to-[#f0e6e6] px-6 py-6 rounded-2xl shadow-lg flex flex-col justify-between items-start border border-[#e0cfcf]">
      <h1 className={`${textClassHeading} py-2 text-[#502a2a] font-bold w-full text-center`}>
        About Shiksha Mahakumbh Abhiyan
      </h1>

      <div className="mb-4 whitespace-pre-line text-justify text-black leading-relaxed">
        <div
          dangerouslySetInnerHTML={{
            __html: isTextExpanded ? text : `${text.substring(0, 800)}.....`,
          }}
        />
        <div className="flex justify-center w-full">
          <button
            onClick={toggleText}
            className="mt-4 px-6 py-2 bg-[#502a2a] text-white rounded-full shadow-md hover:bg-[#7a4343] transition-all duration-300"
          >
