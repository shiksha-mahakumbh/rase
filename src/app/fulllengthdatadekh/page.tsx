...
const Page: React.FC = () => {
  ...
  const [formDataList, setFormDataList] = useState<FullLengthPaperFormData[]>(
    []
  );
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const colRef = collection(db, "FullLengthSubmissionData");
        const querySnapshot = await getDocs(colRef);

        const dataList: FullLengthPaperFormData[] = [];

        querySnapshot.forEach((doc) => {
          const data = doc.data() as FullLengthPaperFormData;
          dataList.push(data);
        });
  ...
  const sendEmail = async (
    email: string,
    formData: FullLengthPaperFormData
  ) => {
    try {
  ...
  const downloadAsPDF = async (formDataList: FullLengthPaperFormData[]) => {
        ...
        <tbody>
          {formDataList.map((formData, index) => (
            <tr key={formData.serial} className="border">
              ...
              <td className="border text-black p-3">
                {formData.CoauthorNames}
              </td>
              <td className="border text-black p-3">
                {formData.CoauthorEmail}
              </td>
              <td className="border text-black p-3">
                {formData.ContactNumber}
              </td>
