...
const Page: React.FC = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        querySnapshot.forEach((doc) => {
          ...
          const registrationTimestamp = doc.data().createdAt;
          if (registrationTimestamp) {
            const registrationDateTime = registrationTimestamp.toDate();
            data.registrationDate = registrationDateTime.toLocaleDateString();
            data.registrationTime = registrationDateTime.toLocaleTimeString();
          } else {
            data.registrationDate = "N/A";
            data.registrationTime = "N/A";
          }
          dataList.push(data);
        });
  ...
  const downloadAsPDF = async (formDataList: NgoData[]) => {
    ...
    const tableColumn = ["serial", "name", "email", "Affiliation", "PhoneNumber", "Services", "registrationDate", "registrationTime"];
    const tableRows: any[][] = [];
  ...
  return (
    <div className="bg-white min-h-screen flex flex-col justify-center items-center mt-4 flex-wrap">
      <Toaster />
      <h2 className="text-primary text-xl font-bold">Volunteer Registration Data</h2>
      <table className="border-collapse border m-6">
        <thead>
          <tr>
            ...
            <th className="border bg-primary text-white font-bold text-base p-3">Affiliation</th>
            <th className="border bg-primary text-white font-bold text-base p-3">PhoneNumber</th>
            <th className="border bg-primary text-white font-bold text-base p-3">Services</th>
            <th className="border bg-primary text-white font-bold text-base p-3">Attachments</th>
            <th className="border bg-primary text-white font-bold text-base p-3">Registration Date</th>
            <th className="border bg-primary text-white font-bold text-base p-3">Registration Time</th>
            <th className="border bg-primary text-white font-bold text-base p-3">QR</th>
            <th className="border bg-primary text-white font-bold text-base p-3">Email Send</th>
          </tr>
        </thead>
        <tbody>
          {formDataList.map((formData, index) => (
            <tr key={formData.serial} className="border">
              ...
              <td className="border text-black p-3">{formData.registrationDate}</td>
              <td className="border text-black p-3">{formData.registrationTime}</td>
      ...
      <button onClick={sendAllEmails} className="bg-primary text-white font-bold py-2 px-4 rounded mt-4 cursor-pointer">
        Send Mails to All
      </button>
    </div>
  );
