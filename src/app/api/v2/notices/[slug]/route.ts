...
import { getNoticeBySlug } from "@/server/services/notice.service";
...
export const GET = createApiHandler(
  async (request: NextRequest, context: { params: Promise<{ slug: string }> }) => {
    ...
    const result = await getNoticeBySlug(slug, locale);
    if (!result) throw new ServiceError("Notice not found", 404, "NOT_FOUND");
