...
export default function RegistrationDetailClient({ id }: Props) {
  ...
  const load = useCallback(async () => {
    if (!isAdminRegistrationIdentifier(id)) {
      setLoadError("Invalid registration ID format.");
      setRecord(null);
      setFetching(false);
      return;
    }
  ...
  const updateStatus = async (
    ...
    if (!canManageStatus(role, permissions) || statusSaving) return;
  ...
  return (
    <div className="space-y-6">
      ...
      {canManageStatus(role, permissions) && (
        <section className="rounded-2xl border bg-white p-5" aria-labelledby="status-management-heading">
          <h2 id="status-management-heading" className="mb-4 font-bold text-primary">
            Status Management
          </h2>
          <fieldset disabled={statusSaving} className="grid gap-4 md:grid-cols-3">
            <StatusSelect
              label="Registration Status"
              value={record.registrationStatus}
              options={record.statusOptions.registration}
              onChange={(v) => void updateStatus("registrationStatus", v)}
            />
            <StatusSelect
              label="Payment Status"
              value={record.paymentStatus}
              options={record.statusOptions.payment}
              onChange={(v) => void updateStatus("paymentStatus", v)}
            />
            <StatusSelect
              label="Accommodation Status"
              value={record.accommodationStatus}
              options={record.statusOptions.accommodation}
              onChange={(v) => void updateStatus("accommodationStatus", v)}
            />
          </fieldset>
          {statusSaving ? (
        ...
        <DetailSection title="Uploaded Documents">
          <ul className="space-y-2">
            {record.documents.map((f) => {
              const safeUrl = sanitizeExternalUrl(f.url);
              return (
                <li key={`${f.fieldName}-${f.url}`}>
                  ...
                  {safeUrl ? (
                    <a
                      ...
                      aria-label={`Download ${f.label} for ${record.registrationId}`}
                    >
                      Download
                    </a>
                  ) : (
...
function DetailSection({
  ...
  return (
    <section className="rounded-2xl border bg-white p-5 shadow-sm" aria-labelledby={headingId}>
      <h2 id={headingId} className="mb-4 text-lg font-bold text-primary">
        {title}
      </h2>
      {children}
    </section>
