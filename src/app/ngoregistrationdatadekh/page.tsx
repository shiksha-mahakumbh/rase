...
const Page: React.FC = () => {
  ...
  const { rows, loading: dataLoading } = useAdminRegistrationData("NGO");
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "NgoData");
    XLSX.writeFile(workbook, "NGORegistrationData.xlsx");
  };
        ...
        <tbody>
          {formDataList.map((formData, index) => (
            <tr key={formData.serial} className="border">
              ...
              <td className="border text-black p-3">{formData.RegistrationNo}</td>
