...
const ALLOWED_EMAILS = ["er.yogeshjangra1@gmail.com", "ykj131@gmail.com","kandarisonal21200@gmail.com"];
...
const AddSpeakerForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const storageRef = ref(storage, `wishesReceived/${imageFile.name}`);
      await uploadBytes(storageRef, imageFile);
      const imageUrl = await getDownloadURL(storageRef);

      if (isEditing && editSpeakerId) {
        const speakerRef = doc(db, "wishesReceived", editSpeakerId);
        ...
        await addDoc(collection(db, "wishesReceived"), {
          name,
          designation,
          place,
          imageSrc: imageUrl,
  ...
  const handleDelete = async (id: string) => {
    try {
      await deleteDoc(doc(db, "wishesReceived", id));
  ...
  return (
    <div className="p-4 max-w-lg bg-black text-white mx-auto ">
      <h2 className="text-2xl  text-white font-bold text-center mb-4">Add/Edit Wishes Received</h2>
